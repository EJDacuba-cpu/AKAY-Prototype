...
export default function AdminDoctorSchedule() {
  ...
  const onDutyCount = schedules.filter(
    (schedule) => schedule.status === "On Duty",
  ).length;
  const fullyBookedCount = schedules.filter(
    (schedule) => schedule.status === "Fully Booked",
  ).length;

  return (
    <DashboardLayout role="admin" title="Doctor Schedule">
      <div className="mb-8">
        <h1 className="text-xl font-bold tracking-tight text-[#0B2E59]">
          Doctor Schedule Overview
        </h1>
        <p className="mt-1 text-sm text-[#6B7280]">
          Monitor RHU doctor schedules, availability, rooms, and specialization
          coverage.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between border-b border-[#E8ECF0] px-6 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-[#9CA3AF]">
                Schedule overview used for RHU coordination and referral
                prioritization.
              </p>
            </div>
                ...
                {schedules.map((schedule) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5">
                      <SpecializationBadge label={schedule.specialization} />
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => updateStatus(schedule.id, "Available")}
                        ...
                        <button
                          onClick={() =>
                            updateStatus(schedule.id, "Fully Booked")
                          }
                        ...
                        <button
                          onClick={() => updateStatus(schedule.id, "On Leave")}
        ...
        <aside className="space-y-6">
            ...
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#9CA3AF]">
                  Specialization coverage overview.
                </p>
              </div>
            </div>

            <div className="space-y-3">
              <CoverageItem label="General Consultation" value="1 doctor" />
              <CoverageItem label="Maternal Care" value="1 doctor" />
              <CoverageItem label="Pediatrics" value="1 doctor" />
              <CoverageItem label="Senior Citizen Care" value="1 doctor" />
            </div>
          </section>
          ...
          <section className="rounded-xl border border-blue-100 bg-blue-50 p-5">
            <p className="text-xs leading-relaxed text-[#4B5563]">
              ...
              admin view helps the MHO monitor doctor availability. RHU staff
              still handles actual patient receiving, consultation workflow, and
              feedback submission.
            </p>
          </section>
        </aside>
