...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('patients', function (Blueprint $table) {
            if (! Schema::hasColumn('patients', 'mother_patient_id')) {
                $table
                    ->foreignId('mother_patient_id')
                    ->nullable()
        ...
        $this->refreshPatientJsonFunction(includeMotherLink: true);
    }
    ...
    public function down(): void
    {
        Schema::table('patients', function (Blueprint $table) {
            if (Schema::hasColumn('patients', 'mother_patient_id')) {
                $table->dropConstrainedForeignId('mother_patient_id');
            }
        });

        $this->refreshPatientJsonFunction(includeMotherLink: false);
    }

    private function refreshPatientJsonFunction(bool $includeMotherLink): void
    {
        if (DB::connection()->getDriverName() !== 'pgsql') {
            return;
        }

        $motherFields = $includeMotherLink
            ? <<<SQL
