 ...
 * --------------------
 * Containment is all-or-nothing. The migration runs in three ordered stages
 ...
 *   2. REVOCATION    - remove object and default privileges.
...
{
    /**
     * PostgreSQL applies GRANT, REVOKE and ALTER DEFAULT PRIVILEGES
     * transactionally, and Migrator::runMigration() wraps up() in
     * $connection->transaction() whenever the grammar supports schema
     * transactions - which the PostgreSQL grammar does - and this property is
     * true. Any RAISE EXCEPTION below therefore rolls the whole phase back,
     * and the migrations table row is never written.
     */
     ...
     * default grants EXECUTE to PUBLIC unless a stored entry overrides it.
     ...
     *
     * Three classes of blocker are detected:
     *
     ...
     *  - default-privilege owners the connected role cannot act as. Every
     *    owner in pg_default_acl is enumerated, not just postgres and
     *    supabase_admin, so an unsafe entry owned by any third role is caught;
     *  - postgres and supabase_admin specifically, which are treated as
     *    required whenever they own objects in public or hold an unsafe
     *    default-privilege entry, because both create objects there.
     *
    ...
    {
        return strtr(<<<'SQL'
DO $akay_containment_preconditions$
DECLARE
...
BEGIN
    ...
    LOOP
        v_blockers := v_blockers || format(
            'default privileges owned by %s in %s grant future objects to a '
    ...
    -- and is relevant here, its default privileges MUST be modifiable, because
    -- the built-in default grants EXECUTE on new functions to PUBLIC even when
    -- pg_default_acl holds no row at all.
    FOR v_row IN
    ...
    IF array_length(v_blockers, 1) > 0 THEN
        RAISE EXCEPTION USING
            ...
            HINT = 'Containment was aborted before any privilege was changed. '
                   'Resolve every listed blocker - see '
                   'docs/database-exposure-containment.md, "Precondition '
                   'failures" - then re-run the migration.';
    END IF;
END
$akay_containment_preconditions$;
...
BEGIN
    ...
    END LOOP;
END
$akay_containment_objects$;
SQL;
    }

    /**
     * Stage 2b. Objects created later.
     *
     * Every pg_default_acl entry with an unsafe grantee is revoked at its own
     * scope - schema-qualified entries with IN SCHEMA, global entries without -
     * because the two are stored and applied separately.
     *
     * postgres and supabase_admin are additionally revoked in schema public
     * unconditionally. pg_default_acl holds no row until a default is modified,
     * so the built-in default (EXECUTE on new functions to PUBLIC) is invisible
     * to the enumeration above and has to be overridden explicitly.
     *
    ...
    {
        return strtr(<<<'SQL'
DO $akay_containment_default_privileges$
DECLARE
    v_grantees text[] := ARRAY['PUBLIC'];
    v_browser_role text;
    v_owner text;
...
BEGIN
    ...
    LOOP
        ...
        -- Stage 1 has already proven membership for every owner that matters:
        -- one that owns objects in public or holds an unsafe stored default.
        -- Reaching this CONTINUE therefore means the role is irrelevant here,
        -- and skipping it is not a silent security failure - stage 3 checks the
        -- effective defaults of postgres and supabase_admin unconditionally and
        -- raises if either would still grant future objects away.
    ...
    END LOOP;
END
$akay_containment_default_privileges$;
SQL, $this->defaultPrivilegeTokens());
    }

    /**
     * Stage 3. Prove containment, or roll the whole transaction back.
     *
     ...
     * membership, and privileges granted to PUBLIC. A browser-facing role that
    ...
    {
        return strtr(<<<'SQL'
DO $akay_containment_assertions$
DECLARE
...
BEGIN
    ...
    LOOP
        v_failures := v_failures || format(
            'future %s created by %s would grant %s to %s',
    ...
    LOOP
        v_failures := v_failures || format(
            'default privileges owned by %s in %s still grant %s objects to %s',
    ...
    IF array_length(v_failures, 1) > 0 THEN
        RAISE EXCEPTION USING
            ...
            HINT = 'Every privilege change in this migration has been rolled '
                   'back. See docs/database-exposure-containment.md, '
                   '"Assertion failures".';
    END IF;
END
$akay_containment_assertions$;
SQL, $this->defaultPrivilegeTokens());
    }
};
